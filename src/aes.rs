use std::convert::TryInto;

const NK: usize = 4;
const NB: usize = 4;
const NR: usize = 10;

const MULTIPLICATION_MATRIX: [[u8; 4]; 4] = [
    [0x2, 0x3, 0x1, 0x1],
    [0x1, 0x2, 0x3, 0x1],
    [0x1, 0x1, 0x2, 0x3],
    [0x3, 0x1, 0x1, 0x2],
];

const INV_MULTIPLICATION_MATRIX: [[u8; 4]; 4] = [
    [0xe, 0xb, 0xd, 0x9],
    [0x9, 0xe, 0xb, 0xd],
    [0xd, 0x9, 0xe, 0xb],
    [0xb, 0xd, 0x9, 0xe],
];

const SBOX: [u8; 256] = [
    ,
];

const INV_SBOX: [u8; 256] = [
    ,
];

const RCON: [u32; 11] = [
    0x00_00_00_00,
    0x01_00_00_00,
    0x02_00_00_00,
    0x04_00_00_00,
    0x08_00_00_00,
    0x10_00_00_00,
    0x20_00_00_00,
    0x40_00_00_00,
    0x80_00_00_00,
    0x1B_00_00_00,
    0x36_00_00_00,
];

// input = 16 words of 8bits
// key = 16 words of 8bits
pub fn cipher(input: &[u8], key: &[u8]) -> Vec<u8> {
    // initialize state
    let mut state = init_state(input);
    println!("STATE: {:X?}", state);

    let w: Vec<u32> = key_expansion(key);
    println!("KEYS: {:x?}", w);

    add_round_key(&mut state, &w[0..4]);
    println!("FIRST_ROUND_KEY: {:X?}", state);

    for round in 1..10 {
        sub_bytes(&mut state, &SBOX);
        println!("SUB_BYTES: {:X?}", state);
        shift_rows(&mut state);
        println!("SHIFT_ROWS: {:X?}", state);
        mix_columns(&mut state, &MULTIPLICATION_MATRIX);
        println!("MIX_COLUMNS: {:X?}", state);
        add_round_key(&mut state, &w[(round * NB)..((round + 1) * NB)]); // next four keys
        println!("ROUND_KEY: {:X?}", state);
    }

    sub_bytes(&mut state, &SBOX);
    println!("LAST_SUB_BYTES: {:X?}", state);
    shift_rows(&mut state);
    println!("LAST_SHIFT_ROWS: {:X?}", state);
    add_round_key(&mut state, &w[(NR * NB)..((NR + 1) * NB)]); // last four keys
    println!("LAST_KEY: {:X?}", state);

    output_state(state)
}

pub fn decipher(input: &[u8], key: &[u8]) -> Vec<u8> {
    let mut state = init_state(input);
    println!("STATE: {:X?}", state);

    let w: Vec<u32> = key_expansion(key);
    println!("KEYS: {:x?}", w);

    // take last keys; 4 last words
    add_round_key(&mut state, &w[(NR * NB)..((NR + 1) * NB)]);
    println!("FIRST_ROUND_KEY: {:X?}", state);

    // reverse order
    for round in (1..10).rev() {
        inv_shift_rows(&mut state);
        println!("INV_SHIFT_ROWS: {:X?}", state);
        sub_bytes(&mut state, &INV_SBOX);
        println!("INV_SUB_BYTES: {:X?}", state);
        add_round_key(&mut state, &w[(round * NB)..((round + 1) * NB)]);
        println!("ROUND_KEY: {:X?}", state);
        mix_columns(&mut state, &INV_MULTIPLICATION_MATRIX);
        println!("INV_MIX_COLUMNS: {:X?}", state);
    }

    inv_shift_rows(&mut state);
    println!("LAST_INV_SHIFT_ROWS: {:X?}", state);
    sub_bytes(&mut state, &INV_SBOX);
    println!("LAST_INV_SUB_BYTES: {:X?}", state);
    add_round_key(&mut state, &w[0..4]); // last four keys
    println!("LAST_KEY: {:X?}", state);

    output_state(state)
}

// convert an input list to a word list
// word1 = u32::from_be_bytes( [ input[0], input[1], input[2], input[3] ] )
// [ word1, word2, word3, word4 ]
fn init_state(input: &[u8]) -> Vec<u32> {
    let mut state: Vec<u32> = vec![0; 4];
    for c in 0..4 {
        let begin = c * NB;
        let end = begin + NB;
        let bytes: [u8; 4] = input[begin..end].try_into().unwrap();
        state[c] = u32::from_be_bytes(bytes);
    }
    state
}

// convert a word list to a list of bytes
// bytes1 = word1.to_be_bytes()
// [ bytes1[0], bytes1[1], bytes1[2], bytes[3], ... ]
fn output_state(state: Vec<u32>) -> Vec<u8> {
    let mut output = vec![0; 16];
    for c in 0..4 {
        let bytes = state[c].to_be_bytes();
        for i in 0..NB {
            output[c * NB + i] = bytes[i];
        }
    }
    output
}

fn add_round_key(state: &mut [u32], key: &[u32]) {
    for c in 0..4 {
        state[c] ^= key[c];
    }
}

fn sub_bytes(state: &mut [u32], sbox: &[u8]) {
    for c in 0..4 {
        let mut bytes = state[c].to_be_bytes();
        for i in 0..4 {
            bytes[i] = sbox[bytes[i] as usize];
        }
        state[c] = u32::from_be_bytes(bytes);
    }
}

// PLEASE improve this shift_rows functions
fn shift_rows(state: &mut [u32]) {
    let mut columns = [
        state[0].to_be_bytes(),
        state[1].to_be_bytes(),
        state[2].to_be_bytes(),
        state[3].to_be_bytes(),
    ];
    for r in 0..NB {
        let mut row = [columns[0][r], columns[1][r], columns[2][r], columns[3][r]];
        row.rotate_left(r);
        columns[0][r] = row[0];
        columns[1][r] = row[1];
        columns[2][r] = row[2];
        columns[3][r] = row[3];
    }
    state[0] = u32::from_be_bytes(columns[0]);
    state[1] = u32::from_be_bytes(columns[1]);
    state[2] = u32::from_be_bytes(columns[2]);
    state[3] = u32::from_be_bytes(columns[3]);
}

fn inv_shift_rows(state: &mut [u32]) {
    let mut columns = [
        state[0].to_be_bytes(),
        state[1].to_be_bytes(),
        state[2].to_be_bytes(),
        state[3].to_be_bytes(),
    ];
    for r in 0..NB {
        let mut row = [columns[0][r], columns[1][r], columns[2][r], columns[3][r]];
        row.rotate_right(r);
        columns[0][r] = row[0];
        columns[1][r] = row[1];
        columns[2][r] = row[2];
        columns[3][r] = row[3];
    }
    state[0] = u32::from_be_bytes(columns[0]);
    state[1] = u32::from_be_bytes(columns[1]);
    state[2] = u32::from_be_bytes(columns[2]);
    state[3] = u32::from_be_bytes(columns[3]);
}

fn mix_columns(state: &mut [u32], mmatrix: &[[u8; 4]; 4]) {
    for c in 0..4 {
        let mut bytes = state[c].to_be_bytes();
        let mut mixed: [u8; 4] = [0; 4];
        for i in 0..4 {
            mixed[i] = dot_product(&mut bytes, &mmatrix[i]);
        }
        state[c] = u32::from_be_bytes(mixed);
    }
}

// multiply two vectors: 1x4 X 4x1
fn dot_product(bytes: &mut [u8], m: &[u8; 4]) -> u8 {
    let mut result: u8 = 0;
    for i in 0..4 {
        result ^= xtime(bytes[i], m[i]);
    }
    result
}

// galois finite field multiplication GF(2^8)
fn xtime(input1: u8, input2: u8) -> u8 {
    let mut result: u8 = 0;
    let mut a = input1;
    let mut b = input2;

    for _ in 0..8 {
        if (b & 0x1) != 0 {
            result ^= a;
        }

        let is_high_bit_set: bool = (a & 0x80) != 0;
        a <<= 1;
        if is_high_bit_set {
            a ^= 0x1b;
        }
        b >>= 1;
    }

    result
}

fn key_expansion(key: &[u8]) -> Vec<u32> {
    let size = NB * (NR + 1);

    let mut w: Vec<u32> = vec![0; size];
    let mut temp: u32;

    for i in 0..4 {
        let word = [key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]];
        w[i] = u32::from_be_bytes(word);
    }

    for i in 4..size {
        temp = w[i - 1].clone();

        if i % 4 == 0 {
            temp = sub_word(rot_word(temp)) ^ RCON[i / NK];
        }

        w[i] = w[i - NK as usize] ^ temp;
    }

    w
}

fn rot_word(input: u32) -> u32 {
    input.rotate_left(8)
}

fn sub_word(input: u32) -> u32 {
    let mut output: [u8; 4] = [0; 4];
    let bytes = input.to_be_bytes();
    for r in 0..4 {
        output[r] = SBOX[bytes[r] as usize]
    }
    u32::from_be_bytes(output)
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn cipher_128() {
        // key expansion test
        let key1 = vec![
            0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf,
            0x4f, 0x3c,
        ];
        let input1 = vec![
            0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37,
            0x07, 0x34,
        ];
        let expected1: Vec<u8> = vec![
            0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a,
            0x0b, 0x32,
        ];
        assert_eq!(expected1, cipher(&input1, &key1));
        assert_eq!(input1, decipher(&expected1, &key1));

        // algo test
        let input2 = vec![
            0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd,
            0xee, 0xff,
        ];
        let key2 = vec![
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0xa, 0xb, 0xc, 0xd, 0xe,
            0xf,
        ];
        let expected2: Vec<u8> = vec![
            0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4,
            0xc5, 0x5a,
        ];
        assert_eq!(expected2, cipher(&input2, &key2));
        assert_eq!(input2, decipher(&expected2, &key2));
    }
}
